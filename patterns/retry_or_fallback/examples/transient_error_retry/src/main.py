...
# Original source: https://github.com/openai/openai-agents-python/blob/25af629b3c877ba3d0e95189141bb636b21167a5/examples/basic/retry.py
# See THIRD_PARTY_NOTICES.md at the repository root for the full license text.

"""Configure and run retries for transient model-request failures.

This example combines SDK retry policies, logs each retry decision, and applies
the resulting settings to an agent run. It demonstrates retry, not fallback.
"""
...
async def main() -> None:
    # 1. Combine the transient-error conditions that should trigger a retry.
    ...
    # 2. Wrap the combined policy so each decision is visible during the tutorial.
    ...
    # 3. Configure the attempt limit and exponential backoff behavior.
    ...
    # 4. Attach the retry settings at both the run and agent levels for comparison.
    ...
    # 5. Run the agent; retry logs appear only when a transient failure occurs.
