...
# Original source: https://github.com/openai/openai-agents-python/blob/25af629b3c877ba3d0e95189141bb636b21167a5/examples/sandbox/basic.py
# See THIRD_PARTY_NOTICES.md at the repository root for the full license text.

"""Run a sandboxed agent that inspects a small generated workspace.

The module builds a manifest and shell-enabled SandboxAgent, creates a Docker or
Modal session, streams the run, and always cleans up the sandbox resources.
"""
...
from .workspace_shell import WorkspaceShellCapability
...
# Build the workspace and the agent that will inspect it.
...
# Load optional backend dependencies only when the selected backend needs them.
def _require_modal_dependency() -> tuple[Any, Any]:
    try:
        ...
        # This import path depends on optional extras.
        raise SystemExit(
            "Modal-backed runs require the optional repo extra.\n"
            "Install the Modal extra for openai-agents 0.22.1; see docs/setup.md"
        ) from exc
...
def _import_docker_from_env() -> Any:
    ...
    try:
        sys.path = [
            entry for entry in sys.path if not _path_resolves_to(entry, script_dir)
        ]
        from docker import from_env as docker_from_env  # type: ignore[import-untyped]
    except Exception as exc:  # pragma: no cover
        # This import path depends on the local Docker setup.
        raise SystemExit(
...
# Create a backend-specific session behind one shared interface.
...
# Run the agent inside the session and stream both text and tool events.
async def main(
    model: str,
    question: str,
    backend: Backend,
...
if __name__ == "__main__":
    # Keep backend and model choices visible so readers can experiment from the CLI.
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", default="gpt-5.6-sol", help="Model name to use.")
    parser.add_argument(
        "--question", default=DEFAULT_QUESTION, help="Prompt to send to the agent."
    )
    parser.add_argument(
        "--backend",
